...
const BASE_URL   = process.env.BASE_URL || 'https://bright-kennel-backend.onrender.com';
...
// Login (supports bcrypt hashes or plaintext stored passwords)
...
// PROTECTED — update pet data (hashes password if provided)
app.post('/update_pet', verifyToken, (req, res) => {
  const {
    petId, name, age, breed, gender, type,
    ownerName, email, password, phone, suitableWeight
  } = req.body;

  if (petId != req.petId) return res.status(403).json({ success: false, message: 'Access denied' });

  function doUpdate(sql, params) {
    pool.query(sql, params, (err, result) => {
      if (err) { console.error('Update DB error:', err); return res.status(500).json({ success: false, message: 'Server error' }); }
      if (!result.affectedRows) return res.json({ success: false, message: 'Pet not found' });
      pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)', [petId, 'Your profile information was updated.'], () => {});
  ...
  if (password && password.trim().length) {
    bcrypt.hash(password, 10, (hashErr, hashed) => {
      if (hashErr) { console.error('Bcrypt hash error:', hashErr); return res.status(500).json({ success: false, message: 'Server error' }); }
      const sql = `
        UPDATE pet
        SET name=?, age=?, breed=?, gender=?, type=?,
            ownerName=?, email=?, password=?, phone=?, suitableWeight=?
        WHERE petId=?
      `;
      doUpdate(sql, [name, age, breed, gender, type, ownerName, email, hashed, phone, suitableWeight, petId]);
    });
  } else {
    const sql = `
      UPDATE pet
      ...
      WHERE petId=?
    `;
    doUpdate(sql, [name, age, breed, gender, type, ownerName, email, phone, suitableWeight, petId]);
  }
});
...
app.post('/update_profile_picture', verifyToken, upload.single('profilePic'), (req, res) => {
  ...
  pool.query('SELECT profilePic FROM pet WHERE petId=?', [petId], (selErr, selRows) => {
    ...
    pool.query('UPDATE pet SET profilePic=? WHERE petId=?', [fullUrl,petId], (updErr,updRes) => {
      ...
      pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)', [petId, 'Your profile picture was updated.'], () => {});
  ...
  pool.query('SELECT quantity FROM cart_items WHERE pet_id=? AND product_id=?',[petId,productId], (findErr, findRows) => {
    ...
    if (findRows.length) {
      ...
      pool.query('UPDATE cart_items SET quantity=? WHERE pet_id=? AND product_id=?',[newQty,petId,productId], (updErr) => {
        ...
        pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Updated cart: product ${productId} now qty ${newQty}.`], ()=>{});
      });
    } else {
      pool.query('INSERT INTO cart_items (pet_id,product_id,quantity) VALUES (?,?,?)',[petId,productId,qtyToInsert], (insErr) => {
        ...
        pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Added to cart: product ${productId} (qty ${qtyToInsert}).`], ()=>{});
      });
    }
  });
});
...
app.post('/remove_from_cart', verifyToken, (req, res) => {
  ...
  pool.query('DELETE FROM cart_items WHERE pet_id=? AND product_id=?',[petId,productId], (err, result) => {
    ...
    pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Removed product ${productId} from cart.`], ()=>{});
  });
});
  ...
  pool.query('SELECT price,discount FROM store WHERE productId=?',[productId], (stockErr, stockRows) => {
    ...
    pool.query(
      ...
      (orderErr) => {
        ...
        pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Your order for product ${productId} qty ${quantity} has been placed.`], ()=>{});
      }
    );
  });
});
  ...
  pool.query(cartSql, [petId], (cartErr, cartRows) => {
    ...
    pool.getConnection((connErr, connection) => {
      ...
      connection.beginTransaction(txErr => {
        if (txErr) { connection.release(); return res.status(500).send({ success:false, message:'Server error' }); }
        connection.query(
          'INSERT INTO orders (pet_id,product_id,quantity,price_at_sale) VALUES ?',
          [ordersData],
          orderErr => {
            if (orderErr) {
              return connection.rollback(() => { connection.release(); res.status(500).send({ success:false, message:'Server error' }); });
            }
            connection.query('DELETE FROM cart_items WHERE pet_id=?',[petId], delErr => {
              ...
              connection.commit(commitErr => {
                ...
                pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Your checkout of ${cartRows.length} items was successful.`], ()=>{});
              });
            });
          }
        );
...
app.post('/appointments', verifyToken, (req, res) => {
  ...
  pool.query(
    ...
    (err, result) => {
      if (err) { console.error('Appointment DB error:', err); return res.status(500).json({ success: false, message: 'Server error.' }); }
      pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[ petId, `Appointment for ${preferred_date} at ${preferred_time} created.` ], ()=>{});
...
app.post('/update_appointment', verifyToken, (req, res) => {
  ...
  pool.query(
    ...
    (err, result) => {
      ...
      pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Appointment #${appointmentId} rescheduled to ${dateOnly} ${timeOnly}.`], ()=>{});
...
app.post('/dietplan', verifyToken, (req, res) => {
  ...
  pool.query('INSERT INTO dietplan (petId,foodList) VALUES (?,?)',[petId, foodList.trim()], (err, result) => {
    ...
    pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, 'A new diet plan has been created for you.'], ()=>{});
  });
});
...
app.put('/dietplan/:dietId', verifyToken, (req, res) => {
  ...
  pool.query('UPDATE dietplan SET foodList=? WHERE dietId=? AND petId=?',[foodList.trim(),dietId,petId], (err, result) => {
    ...
    pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Your diet plan (#${dietId}) was updated.`], ()=>{});
  });
});
app.delete('/dietplan/:dietId', verifyToken, (req, res) => {
  ...
  pool.query('DELETE FROM dietplan WHERE dietId=? AND petId=?',[dietId,petId], (err, result) => {
    ...
    pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Your diet plan (#${dietId}) was deleted.`], ()=>{});
  });
});

// Costs (public endpoints as in your code)
...
app.post('/reminders', verifyToken, (req, res) => {
  ...
  pool.query('INSERT INTO todoreminder (petId,reason,reminderDate,reminderTime) VALUES (?,?,?,?)',
    ...
    (err, result) => {
      ...
      pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Reminder set for ${reminderDate} at ${reminderTime}.`], ()=>{});
    }
  );
});
app.put('/reminders/:id', verifyToken, (req, res) => {
  ...
  const { reason, reminderDate, reminderTime } = req.body;
  if (!reason||reason.trim().length<3||!reminderDate||!reminderTime||isNaN(new Date(`${reminderDate}T${reminderTime}`).getTime())) {
    return res.status(400).send({ success:false, message:'Invalid input' });
  }
  pool.query(
    ...
    (err, result) => {
      ...
      pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Reminder (#${reminderId}) rescheduled.`], ()=>{});
    }
  );
});
app.delete('/reminders/:id', verifyToken, (req, res) => {
  ...
  pool.query('DELETE FROM todoreminder WHERE reminderId=? AND petId=?',[reminderId,petId], (err, result) => {
    ...
    if (!result.affectedRows) return res.status(404).send({ success:false, message:'Not found' });
    res.send({ success:true });
    pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `Reminder (#${reminderId}) cancelled.`], ()=>{});
  });
});
...
app.post('/activities_data2', verifyToken, (req, res) => {
  ...
  pool.query(sql, [petId], (err, rows) => {
    if (err) { console.error('Activities query error:', err); return res.status(500).json({ success: false, message: 'Server error' }); }
...
app.post('/activities', verifyToken, (req, res) => {
  ...
  pool.query('INSERT INTO activities (petId, activityType, duration, date) VALUES (?, ?, ?, ?)',
    [req.petId, activityType, duration, activityDate],
    (err, result) => {
      if (err) { console.error('Insert activity error:', err); return res.status(500).json({ success: false, message: 'Server error' }); }
...
app.put('/admin/appointments/:id/status', (req, res) => {
  ...
  pool.query('UPDATE appointments SET status=? WHERE appointmentId=?',[status,appointmentId], (err,result) => {
    ...
    pool.query('SELECT petId FROM appointments WHERE appointmentId=?',[appointmentId], (err2,rows2) => {
      if (err2||!rows2.length) return res.json({ success:true, message:'Status updated (no notification)' });
...
app.post('/adminVetvisit', (req, res) => {
  ...
  pool.query(sql, params, (err, result) => {
    ...
    pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[petId, `New vet visit recorded for ${date}.`], ()=>{});
  });
});
...
app.put('/adminVetvisit/:visitId', (req, res) => {
  ...
  if (!visitedReason||!date||currentWeight==null) return res.status(400).json({ success:false, message:'visitedReason, date & currentWeight required' });
  const sql=`
...
app.delete('/adminVetvisit/:visitId', (req, res) => {
  const vid=Number(req.params.visitId);
  pool.query('DELETE FROM vetvisit WHERE visitId=?',[vid], (err) => {
    if (err) return res.status(500).json({ success:false, message:'Server error' });
    res.json({ success:true });
  });
});
app.get('/admin/orders', (req, res) => {
  ...
  `;
  const params = [];
  if (startDate && endDate) { sql += ' WHERE DATE(o.order_date) BETWEEN ? AND ?'; params.push(startDate, endDate); }
...
app.post('/admin/orders/:orderId/accept', (req, res) => {
  ...
  pool.getConnection((connErr, conn) => {
    ...
    conn.beginTransaction(txErr => {
      ...
      conn.query('SELECT pet_id,product_id,quantity,price_at_sale,order_date FROM orders WHERE order_id=? FOR UPDATE',[oid],
        (selErr, selRows) => {
          if (selErr || !selRows.length) {
            return conn.rollback(() => { conn.release(); selErr ? res.status(500).json({ success:false, message:'Server error' }) : res.status(404).json({ success:false, message:'Order not found' }); });
          }
          const o = selRows[0];
          ...
          conn.query('SELECT qty FROM store WHERE productId=? FOR UPDATE',[o.product_id], (stErr, stRows) => {
            if (stErr || !stRows.length || stRows[0].qty < o.quantity) {
              return conn.rollback(() => { conn.release(); const msg = stErr ? 'Server error' : 'Insufficient stock'; res.status(stErr ? 500 : 400).json({ success:false, message:msg }); });
            }
            conn.query('INSERT INTO sales (petId,productId,qty,cost,date) VALUES (?,?,?,?,?)',
              [o.pet_id, o.product_id, o.quantity, totalCost, o.order_date],
              insErr => {
                if (insErr) return conn.rollback(() => { conn.release(); res.status(500).json({ success:false, message:'Server error' }); });
                conn.query('UPDATE store SET qty=qty-? WHERE productId=?',[o.quantity,o.product_id], (updErr,updRes) => {
                  if (updErr || updRes.affectedRows===0) return conn.rollback(() => { conn.release(); res.status(500).json({ success:false, message:'Server error' }); });
                  conn.query('DELETE FROM orders WHERE order_id=?',[oid], delErr => {
                    if (delErr) return conn.rollback(() => { conn.release(); res.status(500).json({ success:false, message:'Server error' }); });
                    conn.commit(cmErr => {
                      if (cmErr) return conn.rollback(() => { conn.release(); res.status(500).json({ success:false, message:'Server error' }); });
                      conn.release();
                      res.json({ success:true, message:'Order accepted and stock updated' });
                      pool.query('INSERT INTO notifications (petId,message) VALUES (?,?)',[o.pet_id, `Your order #${oid} has been accepted.`], ()=>{});
                    });
                  });
                });
              }
...
app.get('/admin/sales', (req, res) => {
  ...
  `;
  const params = [];
  if (startDate && endDate) { sql += ' WHERE DATE(s.date) BETWEEN ? AND ?'; params.push(startDate, endDate); }
